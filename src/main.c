...
void printHello(int nextX, int nextY) {
    ...
    printf("*");  // Imprime um caractere na nova posição (originalmente um HelloWorld)
}
...
int main() {
    ...
    while (ch != 10) {  // Continua até que "Enter" seja pressionado
        ...
        if (keyhit()) {  // Se uma tecla foi pressionada
            ch = readch();  // Lê o caractere pressionado
            printKey(ch);  // Mostra o código da tecla
        ...
        if (timerTimeOver() == 1) {  // Verifica se o temporizador terminou
            ...
            if (newX >= (MAXX - strlen("*") - 1) || newX <= MINX + 1) 
